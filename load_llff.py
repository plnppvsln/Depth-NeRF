...
def load_colmap_depth(basedir, factor=8, bd_factor=.75):
    ...
    # Получаем отсортированный список ключей изображений
    # (изначально код предполагал, что индексы изображений идут без пропусков и отсортированы)
    ...
    for idx, image_id in enumerate(image_ids):
        ...
        for i in range(len(images[image_id].xys)):
            ...
            depth = (poses[idx-1,:3,2].T @ (point3D - poses[idx-1,:3,3])) * sc
            if depth < bds_raw[idx-1,0] * sc or depth > bds_raw[idx-1,1] * sc:
                continue
